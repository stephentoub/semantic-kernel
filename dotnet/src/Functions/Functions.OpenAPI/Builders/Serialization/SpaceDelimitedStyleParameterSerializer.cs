...
internal static class SpaceDelimitedStyleParameterSerializer
{
    ...
    public static string Serialize(RestApiOperationParameter parameter, string argument)
    {
        ...
        {
            throw new KernelException($"Serialization of Rest API operation parameters of type `{parameter.Type}` is not supported for the `{RestApiOperationParameterStyle.SpaceDelimited}` style parameters. Parameter name `{parameter.Name}`.");
        }
